  ...
  if (status === "expired") {
    return (
      <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
        <p className="font-medium text-slate-700">مهلت پرداخت تمام شده است</p>
        <p className="mt-1 text-slate-500">تاریخ‌های این رزرو آزاد شده‌اند. می‌توانید دوباره رزرو کنید.</p>
      </div>
    );
  }
  ...
  if (status === "pending_payment" && hasPayment) {
    return (
      <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
        ...
        {receiptPath ? (
          <Link href={receiptPath} target="_blank" className="mt-2 inline-block font-semibold text-blue-700">
            مشاهده فیش
          </Link>
        ) : null}
      </div>
    ...
    return (
      <div className="mt-3 space-y-2 rounded-xl border border-amber-200 bg-amber-50 p-3">
        ...
        <Link
          ...
          className="btn-primary inline-flex min-h-12 w-full items-center justify-center text-base sm:w-auto"
        >
